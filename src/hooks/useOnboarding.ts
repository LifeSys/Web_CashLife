...
import { Account, CreditCard, Person, Subscription, Wallet } from '@/types';
...
export function useOnboarding() {
  ...
  const wallets = useSWR(uid ? ['wallets', uid] : null, () => uid ? walletService.getAll(uid) : []);
  ...
  const run = async <T,>(message: string, action: (uid: string) => Promise<T>) => {
    try {
      ...
      await Promise.all([accounts.mutate(), cards.mutate(), subscriptions.mutate(), wallets.mutate(), people.mutate()]);
      return result;
    } catch (error) {
  ...
  return {
    ...
    people: people.data ?? [],
    loading: accounts.isLoading || cards.isLoading || subscriptions.isLoading || wallets.isLoading || people.isLoading,
    ...
    createWallet: (wallet: Omit<Wallet, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy'>) => run('Billetera guardada', (uid) => walletService.create(uid, wallet)),
