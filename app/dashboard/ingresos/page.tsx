      ...
      <div className="grid gap-4">
        {incomes.map((item) => (
          <article key={item.id} className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-start justify-between gap-4"><div><h2 className="font-bold">{item.description}</h2><p className="text-sm text-muted-foreground">{'status' in item ? `Estado: ${item.status}` : 'Registro financiero'}</p></div><strong>{formatCurrency(item.amount)}</strong></div>
          </article>
        ))}
        {!incomes.length && <div className="rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">Aún no hay registros. Usa el botón “Nuevo registro” para empezar.</div>}
      </div>
    </div>
  );
}
