      ...
      <div className="grid gap-4">
        {obligations.map((item) => (
          <article key={item.id} className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-start justify-between gap-4"><div><h2 className="font-bold">{item.description}</h2><p className="text-sm text-muted-foreground">{'status' in item ? `Estado: ${item.status}` : 'Registro financiero'}</p></div><strong>{formatCurrency(item.pendingBalance)}</strong></div>
          </article>
        ))}
