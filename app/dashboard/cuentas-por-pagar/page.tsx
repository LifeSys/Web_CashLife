...
export default function Page() {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-border bg-card p-4"><p className="text-sm text-muted-foreground">Pendiente por pagar</p><p className="text-3xl font-bold">{formatCurrency(total)}</p></div><div className="rounded-xl border border-border bg-card p-4"><p className="text-sm text-muted-foreground">Ya pagado</p><p className="text-3xl font-bold text-emerald-500">{formatCurrency(paid)}</p></div></div>
      <div className="grid gap-4">
        {obligations.map((item) => (
          <article key={item.id} className="rounded-xl border border-border bg-card p-4">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div><h2 className="font-bold">{item.description}</h2><p className="text-sm text-muted-foreground">{item.creditorName} · {item.creditorType} · Estado: {item.status}</p></div>
